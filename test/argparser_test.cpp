...
TEST(ArgParserTestSuite, EverythingTest) {
    ArgParser parser("My Parser");
    parser.AddHelp('h', "help", "Some description about program");
    parser.AddIntArgument('n', "number", "number to check smth");
    parser.AddFlag('s', "flag1", "variant1");
    bool f;
    parser.AddFlag('p', "flag2", "variant2").StoreValue(f);
    std::string input_path;
    std::string output_path;
    parser.AddStringArgument('i', "input", "path to the input file").StoreValue(input_path);
    parser.AddStringArgument('o', "output", "output file path").Positional().StoreValue(output_path);
    ASSERT_TRUE(parser.Parse(SplitString("app -i=file.tsv -n 15 output.cpp -s --flag2")));
    ASSERT_EQ(input_path, "file.tsv");
    ASSERT_EQ(output_path, "output.cpp");
    ASSERT_EQ(parser.GetIntValue("number"), 15);
    if (parser.GetFlag("flag1")) {
        parser.AddArgument<long long>('l', "long", "long number");
        parser.AddArgument<double>('d', "double", "double number");
    }
    if (f) {
        parser.AddArgument<float>('f', "float");
    }
    ASSERT_TRUE(parser.Parse(SplitString("app -i output.cpp input.cpp --number 239 -l=12345678998765 --double=2.4567 -f 234.349")));
    ASSERT_EQ(input_path, "output.cpp");
    ASSERT_EQ(output_path, "input.cpp");
    ASSERT_EQ(parser.GetIntValue("number"), 239);
    ASSERT_EQ(parser.GetValue<long long>("long"), 12345678998765);
    ASSERT_EQ(parser.GetValue<double>("double"), 2.4567);
    ASSERT_EQ(parser.GetValue<float>("float"), 234.349f);
}
...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    // );
    ASSERT_TRUE(parser.Parse(SplitString("app -h")));
    std::cout << std::endl;
}